...
1. 15% of the entries in the news, blogs and Twitter corpora for the **training** set,
2. 5% of the entries in the news, blogs and Twitter corpora for the **test** set.
...
save(summary.train, summary.test, file="./summaries.Rdata")
```
...
```{r training tokens}
pander(data.table(no_tokens=ntoken(tokens.train)))
```
While the training set is made of:
```{r test tokens}
pander(data.table(no_tokens=ntoken(tokens.test)))
```
